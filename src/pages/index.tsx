...
export default function Home({ poolsCount, betsCount, usersCount }: HomeProps) {
  ...
  async function createPool(event: FormEvent) {
    ...
    try {
      ...
      alert(
        "Bolão criado com sucesso! Código do bolão copiado para a área de transferência!"
      );
  ...
  return (
    <div className="max-w-[1124px] h-screen mx-auto grid grid-cols-2 gap-28 items-center">
      ...
      <Image
        src={phonesImg}
        alt="Dois celulares exibindo uma prévia do app mobile"
        quality={100}
      />
    </div>
  );
...
export const getServerSideProps = async () => {
  const [poolCountResponse, betCountResponse, userCountResponse] =
    await Promise.all([
      api.get("pools/count"),
      api.get("bets/count"),
      api.get("users/count"),
  ...
  return {
    props: {
      ...
      betsCount: betCountResponse.data.count,
